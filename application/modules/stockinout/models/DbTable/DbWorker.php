...
class Stockinout_Model_DbTable_DbWorker extends Zend_Db_Table_Abstract
{
	...
	function addWorker($data)
	{
		try {
			...
			$existing = $this->ifWorkorExisting($data);
			...
			if (empty($existing)) {
				...
				Application_Form_FrmMessage::Sucessfull("DATA_EXISTING", "/stockinout/staff/add", 2);
			}
		} catch (Exception $e) {
	...
	function updateWorker($data)
	{
		try {
			...
			if (empty($existing)) {
				...
				Application_Form_FrmMessage::Sucessfull("DATA_EXISTING", "/stockinout/staff/index", 2);
			}
	...
	function ifWorkorExisting($data)
	{
