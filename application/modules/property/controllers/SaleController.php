<?php
class Property_saleController extends Zend_Controller_Action {
...
// 			$this->view->list=$list->getCheckList(2, $collumns, $rs_rows,array('buy_no'=>$link_info,'land_type'=>$link_info,'width'=>$link_info,'height'=>$link_info,'size'=>$link_info),0);
  ...
  function addAction()
  {
		if($this->getRequest()->isPost()){
			...
			try {
				$_db = new Property_Model_DbTable_DbSale();
				$_db->addSale($_data);
				if(!empty($_data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/property/sale");
				}else{
		...
		$frm = new Property_Form_FrmSale();
		$frm_sale=$frm->FrmSale();
		Application_Model_Decorator::removeAllDecorator($frm_sale);
		$this->view->frm_sale = $frm_sale;
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()){
			...
			$_data['id'] = $id;
			try{
				$_db->updateSale($_data);
				Application_Form_FrmMessage::Sucessfull("UPDATE_SUCCESS","/property/sale");
			}catch (Exception $e) {
		...
		$rs = $_db->getSaleById($id);
		$this->view->rs = $rs;
// 		$row_detail = $_db->getSalePropertyDetail($id);
// 		$identity='';
// 		foreach ($row_detail  as $rss){
// 			if (!empty($identity)){
// 				$identity = $identity.",".$rss['house_id'];
// 			}else{$identity = $rss['house_id'];
// 			}
// 		}		
// 		$this->view->identity = $identity;
// 		$this->view->rowdetail = $row_detail;
		$frm = new Property_Form_FrmSale();
		$frm_sale=$frm->FrmSale($rs,1);
		Application_Model_Decorator::removeAllDecorator($frm_sale);
		$this->view->frm_sale = $frm_sale;
