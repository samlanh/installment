...
class Invpayment_Model_DbTable_DbIssueCheque extends Zend_Db_Table_Abstract
{
	...
	function getAllIssueChequePayment($search)
	{
		...
		if (!empty($search['statusWithdraw'])) {
			if ($search['statusWithdraw'] == 2) {
				$search['statusWithdraw'] = 0;
			}
	...
	function getAllReceiveChequePayment($search)
	{
		$db = $this->getAdapter();
		$dbGb = new Application_Model_DbTable_DbGlobal();
		...
		$sql = "
			...
			";
		$sql .= ",(SELECT  CONCAT(COALESCE(u.first_name,'')) FROM rms_users AS u WHERE u.id=reCh.drawUserId LIMIT 1 ) AS byUser";
		$sql .= " FROM `st_receive_cheque` AS reCh
					LEFT JOIN `st_payment` AS pt ON pt.id = reCh.paymentId 
					LEFT JOIN `st_supplier` AS spp ON spp.id = pt.supplierId 
				WHERE reCh.drawUserId IS NOT NULL
		";

		$where = "";
		$from_date = (empty($search['start_date'])) ? '1' : " reCh.withdrawDate >= '" . $search['start_date'] . " 00:00:00'";
		$to_date = (empty($search['end_date'])) ? '1' : " reCh.withdrawDate <= '" . $search['end_date'] . " 23:59:59'";
