...
class Invpayment_StatementproductController extends Zend_Controller_Action {
	...
	function addAction(){
    	...
    	$frm = new Invpayment_Form_FrmStatementproduct();
	...
	function getallinvoiceAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$db = new Invpayment_Model_DbTable_DbInvoiceStatement();
			$_row =$db->getInvoiceId($data);
			print_r(Zend_Json::encode($_row));
			exit();
			
		}
	}
	function getInvoiceInfoAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$db = new Invpayment_Model_DbTable_DbInvoiceStatement();
			$_row =$db->getInvoiceDetail($data);
			print_r(Zend_Json::encode($_row));
			exit();
