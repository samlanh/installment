<?php 
Class Incexp_Form_FrmExpensePayment extends Zend_Dojo_Form {
	protected $tr;
...
//				'class'=>'fullside height-text',));
		...
		$options = $_dbgb->getAllBranchName(null,1);
...
// 		$_date_payment->setValue($_date);
		...
		$_arr_optview = array();
		$optionView = $_dbgb->getViewById(26);
		...
		$_cheque_no->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside height-text',
				'missingMessage'=>$this->tr->translate("Forget Enter Cheque No")
		));
...
//		$_branch_search->setValue($request->getParam("branch_search"));
		...
		$options = $_dbgb->getAllBranchName(null,1);
		$_branch_search->setMultiOptions($options);	
		$_branch_search->setValue($request->getParam("branch_search"));
		...
		$_bypuchase_no->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside height-text',
				'placeholder'=>$this->tr->translate("FILTER_BY_RECEIPT_NO"),
				'missingMessage'=>$this->tr->translate("Forget Enter Receipt No")
		));
		...
		if(!empty($data)){
			...
			$_cheque_no->setValue($data["cheque_no"]);
