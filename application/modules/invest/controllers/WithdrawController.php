<?php
class Invest_WithdrawController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$db = new Invest_Model_DbTable_DbWithdraw();
			$rs_rows= $db->getAllInvestmentReceipt($search);
			
			$list = new Application_Form_Frmtable();
			$collumns = array("RECIEPT_NO","INVESTOR","INVEST_NO","PRINCIPLE_PAID","AMOUNT_RETURN","TOTAL_AMOUNT_RETURN","RECEIVE_AMOUNT","PAY_DATE","DATE_INPUT","PAYMENT_OPTION","BY_USER","STATUS");
  ...
  function addAction()
  {
		if($this->getRequest()->isPost()){
			...
			try {
				...
				$_dbmodel = new Invest_Model_DbTable_DbWithdraw();
				$_dbmodel->addInvestmentWithdrawal($_data);
				if(!empty($_data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/invest/withdraw");
				}else{
					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/invest/withdraw/add");
				}
			}catch (Exception $e) {
	...
	function deleteAction(){
		...
		if (r == true) {";
			echo "window.location ='".Zend_Controller_Front::getInstance()->getBaseUrl()."/invest/withdraw/deletereceipt/id/$id';";
			echo "}";
			echo "else {";
			echo "window.location ='".Zend_Controller_Front::getInstance()->getBaseUrl()."/invest/withdraw';";
			echo "}
		</script>";
	}
	function deletereceiptAction(){
		...
		try {
			...
			if(!empty($rs)){
				...
				if(!empty($row)){
					$db->deleteReceipt($id);
					Application_Form_FrmMessage::Sucessfull("DELETE_SUCCESS","/invest/withdraw");
				}else{
	...
	function getScheduleInvestorAction(){
		if($this->getRequest()->isPost()){
			...
			$db = new Invest_Model_DbTable_DbWithdraw();
			$row = $db->getInvestPaymentScheduleByID($data);
	...
	function getAllinvestdetailAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$db = new Invest_Model_DbTable_DbWithdraw();
			$row = $db->getAllInvestPaymentScheduleById($data);
			print_r(Zend_Json::encode($row));
			exit();
		}
	}
	function getInvesthaswithdrawAction(){//បង្ហាញប្រវត្តប្រាក់បានបងសរុបtab3
		if($this->getRequest()->isPost()){
			...
			$row = $db->getInvestHasPayByID($investment_id);
			print_r(Zend_Json::encode($row));
			exit();
		}
	}
