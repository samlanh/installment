...
class Invest_Model_DbTable_DbBroker extends Zend_Db_Table_Abstract
{
    protected $_name = 'rms_broker';
	...
	public function addBroker($_data){
		try{
			$brokerCode = $this->getNewBrokerCode();
		    $_arr=array(
				'code'	  				=> $brokerCode,
	    		'name'	  				=> $_data['name'],
				'sex'	      			=> $_data['sex'],
		    	...
		    	'percent_income'	    => $_data['percent_income'],
		    ...
		    if (!empty($photo_name)){
		    	...
		    	$image_name_stu = "broker-profile_".date("Y").date("m").date("d").time().".".end($tem);
	...
	function getBrokerById($id){
		$db = $this->getAdapter();
		$sql="SELECT i.* FROM rms_broker AS i WHERE i.id = $id LIMIT 1";
