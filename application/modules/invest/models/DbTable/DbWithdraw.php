...
class Invest_Model_DbTable_DbWithdraw extends Zend_Db_Table_Abstract
{
    ...
    function getAllInvestmentReceipt($search = null){
    	try{
    		...
    		$sql.="  FROM `rms_investor_withdraw` AS w,	`rms_investor` AS v WHERE v.id = w.investor_id ";
	    	...
	    	if(!empty($search['investor_search'])){
	    		$where.=" AND w.investor_id = ".$search['investor_search'];
	    	}
    ...
    function getAllInvestment($is_complete=0){
    	$db = $this->getAdapter();
    	$sql ="SELECT i.id,
    ...
    public function addInvestmentWithdrawal($_data){
    	try{
    		...
    		$_arr=array(
    				...
    				'principle_paid'		=>$_data['principle_paid'],
    		...
    		$rows = $this->getInvestorScheduleById($investment_id, 1);
    		if(!empty($rows)){
    			...
    			foreach ($rows AS $key => $row){
    				...
    				$after_outstanding = $row['beginning_balanceafter'];
    				$after_payment_after = $row['total_paymentafter'];
    				$after_principal = $row['principle_after'];
...
//     				}
    				...
    				if($option_pay!=2){//ព្រោះបញ្ចូលជា Extra payment hz
    					$arra = array(
    							'beginning_balanceafter'=>$after_outstanding-$paid_principal,
    							"principle_after"=>$after_principal,
    							'interest_amountafter'=>$after_interest,
    							'total_paymentafter'=>	$after_principal+$after_interest,
    ...
    function getInvestorScheduleById($investment_id,$orderby=1){
    	$db = $this->getAdapter();
    	$sql="SELECT * FROM  rms_investment_detail WHERE status=1 AND is_complete=0 AND investment_id=$investment_id ";
    	if($orderby==1){
    		$sql.=" ORDER BY id ASC ";
    	}else{
    ...
    function getInvestPaymentScheduleByID($data){//tab1
    	$db = $this->getAdapter();
    	$sql ="SELECT
	    	s.*,
	    	s.date AS sold_date,
	    	DATE_FORMAT(s.date, '%d-%m-%Y') AS `buy_date`,
	    	(SELECT iv.phone FROM `rms_investor` AS iv WHERE iv.id=s.investor_id LIMIT 1) AS phone,
	    	(SELECT iv.name FROM `rms_investor` AS iv WHERE iv.id=s.investor_id LIMIT 1) AS investor_name,
    ...
    function getAllInvestPaymentScheduleById($data){//tab2
    	$db = $this->getAdapter();
    	$loan_number= $data['investment_id'];
    	$sql = "SELECT *,
    		DATE_FORMAT(scd.date, '%d-%m-%Y') AS `date_payment`
    	FROM
	    	rms_investment as s ,
	    	rms_investment_detail as scd
    	WHERE
	    	scd.status=1
	    	AND s.id=scd.investment_id
	    	AND s.id = $loan_number
	    	ORDER BY scd.time ASC ";
    	return $db->fetchAll($sql);
    }
    
    	public function getInvestHasPayByID($investment_id){//tab3
	    	$db= $this->getAdapter();
	    	$sql="
	    	SELECT
		    	crm.*,
    	...
    	function getReceiptByID($receipt_id){
    		$db = $this->getAdapter();
    		$sql="SELECT * FROM rms_investor_withdraw WHERE id =$receipt_id ";
    	...
    	function checkifExistingDelete($id){
    		$db = $this->getAdapter();
    		$sql="SELECT recieve_amount FROM `rms_investor_withdraw` WHERE id=$id AND recieve_amount>0";
    	...
    	function deleteReceipt($receipt_id){
    		...
    		try{
    			...
    			FROM
    			`rms_investor_withdraw_detail` AS crmd,
    			rms_investor_withdraw AS crm
    			WHERE
    				...
    				foreach ($receipt_money_detail as $rs){
    					if(!empty($rs['lfd_id'])){
    						...
    						if($updated==0){//ករណីមានលុបខ្លះ ត្រូវបញ្ចូលឡើងវិញ
    							$sql="SELECT (time) FROM rms_investment_detail WHERE status=1 AND investment_id=$investment_id ORDER BY time DESC ";
