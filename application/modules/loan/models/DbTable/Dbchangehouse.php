...
class Loan_Model_DbTable_Dbchangehouse extends Zend_Db_Table_Abstract
{
   ...
   function getAllChangeHouse($search){
   	...
   	$sql="SELECT 
		...
		c.name_kh,
		(SELECT CONCAT(COALESCE(p.land_address,''),',',COALESCE(p.street,'')) FROM `ln_properties` AS p WHERE p.id=cp.from_houseid LIMIT 1) from_property,
		...
		(SELECT CONCAT(COALESCE(p.land_address,''),',',COALESCE(p.street,'')) FROM `ln_properties` AS p WHERE p.id=cp.to_houseid LIMIT 1) to_propertype,
