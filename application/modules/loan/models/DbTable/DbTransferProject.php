...
//     		$this->update($arr, $where);
    ...
    public function getAllLandInfo($to_branch_id){
    	$db = $this->getAdapter();
    	$sql="SELECT 
				  `id`,
				  `land_address` AS name 
				FROM
				  `ln_properties` 
				WHERE status = 1 
				  AND `land_code` != '' 
				  AND `is_lock` = 0 
				  AND branch_id = $to_branch_id ";//just concate
    	
    	return $db->fetchAll($sql);
    	
    }
    
    public function getLandInfoTransfer($land_id){
    	$db = $this->getAdapter();
    	$sql="SELECT *,(select type_nameen from ln_properties_type where id = ln_properties.property_type) as property_type_transfer from ln_properties where id = $land_id ";//just concate
