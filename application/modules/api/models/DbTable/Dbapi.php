...
//     }
    ...
    public function getAllCollectPayment($search=null){
		...
		$sql= $statement['sql'];
		$sql.=" 
			,FORMAT(pd.total_payment_after,2) AS total_payment_after,
			DATE_FORMAT(pd.date_payment,'%d-%m-%Y') AS date_payment ";
    	$where = $statement['where'];
    	$where.="  AND `pd`.`is_completed` = 0 ";
		
    	
    	$where.= (empty($search['end_date']))? '1': " AND pd.date_payment <= '".$search['end_date']." 23:59:59'";
    	$order=" GROUP BY pd.id ORDER BY pd.date_payment ASC ";
