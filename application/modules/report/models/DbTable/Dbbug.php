...
class Report_Model_DbTable_Dbbug extends Zend_Db_Table_Abstract
{
      ...
      function getScheduleCompletednotUpdate(){
      	...
      	$sql="SELECT s.id,
				...
				s.price_sold
			      	 FROM `ln_sale` AS s,`ln_saleschedule` AS ss WHERE s.status =1 AND s.is_cancel=0 AND s.payment_id!=1 AND s.payment_id!=2
			      	 AND ss.principal_permonth<=0 AND ss.total_payment_after=0 AND ss.is_completed=0 group by ss.id";
      	return $db->fetchAll($sql);
      }
