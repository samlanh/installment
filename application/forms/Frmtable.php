    ...
    public function getCheckList($delete=0, $columns,$rows,$link=null,$editLink="", $class='items', $textalign= "left", $report=false, $id = "exportExcel")
    {
    	...
    	$head='<form name="list">
    				<div class="dataTables_scrollBody" style="position: relative;  width: 100%; background:#fff;   ">
    					<table border="1" id="datatable-responsive" style="  border-collapse: collapse;   border-color: #ddd;"  class="responsiveTable display nowrap dataTable dtr-inline collapsed" cellspacing="0" width="100%" >
    	';
    	$col_str='';
    	$col_str .='<thead><tr>';
    	...
    	$row_str='<tbody>';
    	//add element rows	
    	if($rows==NULL) return $head.$col_str.'</table></div><center style="font-size:18pt;"><label id="data_table">'.$tr->translate("NO_DATA").'</label></center></form>';
    	$temp=0;
    	/*------------------------Check param id----------------------------------*/
