...
class Application_Form_FrmAdvanceSearchStock extends Zend_Dojo_Form
{
	...
	public function AdvanceSearch($data = null, $type = null)
	{
		...
		if (!empty($rowPurchasingType))
			foreach ($rowPurchasingType as $row) {
		...
		if (!empty($rowInvoiceType))
			foreach ($rowInvoiceType as $row) {
		...
		$paymentMethod->setAttribs(
			array(
				'dojoType' => 'dijit.form.FilteringSelect',
				'placeholder' => $this->tr->translate("PAYMENT_METHOD"),
				'required' => 'false',
				'class' => 'fullside',
			)
		);
